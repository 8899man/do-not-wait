...
class ZjEstimatePopup extends React.Component {
 ...
 handleOk=()=>{
    ...
    if(this.state.myValue === "" ){
        ...
        return;
    }else if(this.selloutStore.reasontext.reasonName === "" && this.state.recordValue.trim() === ""){
       message.warn("请选择原因"); 
       return; 
    }else if(this.state.recordValue !== "" && this.state.recordValue.length >100){
      message.warn("输入的原因不能超过100个字"); 
       return; 
    }else{
        let quantity = this.state.myValue;//变动数量
        let textareaReason =  this.state.recordValue;//变动原因textarea 
        let changeReason = this.selloutStore.reasontext.reasonName === "" ? textareaReason : this.selloutStore.reasontext.reasonName; 
 ...
 reachValue=(e)=>{ 
  ...
  render() { 
    ...
    return ( 
       <div>
          <Modal title={this.selloutStore.addorreducename}
              visible={true}
              closable={false}
              maskClosable={false}
              onCancel={this.handleCancel}
              onOk={this.handleOk}
              width={840} wrapClassName="zj-estimate-popup-modal"
            >
           <div className="zj-estimate-title">{this.selloutStore.afterobj.productName}</div>
           <div className="zj-estimate-left">
                  ...
                  <MyScroll > 
                    {(() =>{
                      if(reasonlist.length) {
                          ...
                          return <div key={reason.DictionaryID}  className={classnames({'each-reason': true, 'each-checked': reason.selected})}  
                                    onClick={()=>{ 
                                      this.selloutStore.selectreason(reason.DictionaryID);  //选中
                                      this.selloutStore.getreasontext(reason.DictionaryID,reason.DictionaryName);  //获取文本 
                                      this.setState({recordValue:''});
                                    }}
                                  > 
                                    {reason.selected && <i className="iconfont icon-icon_checkbox_sel"></i> }
              ...
              <div className="zj-estimate-otherreason">
                <p>其他原因</p>
                <textarea placeholder="可输入原因" onChange={this.reachValue}  value={this.state.recordValue}></textarea>
              </div>
           </div>
           <div className="zj-estimate-right">
            <div className="zj-right-keyboard">
              <CommonKeyboardNum  Whetherpoint={this.selloutStore.afterobj.needWeigh ? true : false} getResult={(value)=>{
