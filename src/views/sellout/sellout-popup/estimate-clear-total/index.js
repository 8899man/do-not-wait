...
function SelloutLb({ selloutStore, sellout,handleClick}) { 
    let optionName; 
    if(sellout.allOptionNameList !==null && sellout.allOptionNameList.length >0){
	 ...
	 return <li  onClick={handleClick} className={sellout.selected === true ? 'each-data click-data' : 'each-data'} >
...
class EstimateClearTotal extends React.Component {
 ...
 constructor(props, context) {
        ...
        this.state={
            ...
            GqEstimate:'',//沽清/删除弹出层
            Menu:'',//右侧点菜组件
            returndate:selloutStore.fillindate === "" ? selloutStore.currentdate : selloutStore.fillindate,//回掉日期
            Import:'',//导入弹出层
        ...
        selloutStore.getselloutList({storeDay:selloutStore.fillindate === "" ? selloutStore.currentdate : selloutStore.fillindate});//执行沽清列表action  0代表全部分类
    ...
    addsellout=()=>{
        ...
        let selloutStore=this.props.selloutStore;  
        let flagt;
        selloutStore.selloutList.map((sell,index)=>{//判断是否选中有数据 
    ...
    importhistory=()=>{
        ...
        <ImportPopup 
            ...
            importok={()=>{ 
                selloutStore.implementImport(this.state.returndate,selloutStore.Importtype);//执行导入action
                this.setState({Import:''});
            }}>
        </ImportPopup>
        });
                ...
                <div className="estimate-main">
                    <div className="estimate-main-search">
                        ...
                        <div className="search-right">
                                ...
                                onClick={()=>{ 
                                    ...
                                    this.setState({Commodity:
                                                   <CommodityIficationPopup 
                                                        ...
                                                        okbutton={()=>{
                                                            this.setState({Commodity:'',
                                                                            defaultvalue:  selloutStore.lastcombination.lastname === ""  ? 
                                                                                '全部':
                                                                                selloutStore.lastcombination.lastname
                                                                });
                                                            selloutStore.getselloutList({
                                                                storeDay:this.state.returndate,
                                                                categoryID: selloutStore.lastcombination.lastid === "" 
                                                                            ? '0'
                            ...
                            <input type="text" placeholder="请输入商品编码/名称/拼音码查询" className="estimate-main-input" value={this.state.recordValue}  onChange={this.reachValue}></input>
                            ...
                            <div className="estimate-list-button">
                                ...
                                {this.state.returndate === selloutStore.currentdate    &&//日期是当前日期就显示【增加/减少/报损/沽清/删除/添加/操作历史】按钮
                                    <div className="normal-button">
                                        <button  className={selloutStore.selloutList.length === 0 && 'disabled' }
                                            onClick={selloutStore.selloutList.length > 0 &&this.addsellout}>增加沽清数量
                                        </button>
                                        
                                        <button  className={selloutStore.Quantity === 0 && 'disabled' }
                                            onClick={selloutStore.Quantity >0 && this.reducesellout}>减少沽清数量 
                                        </button>
                                        <button className={selloutStore.Quantity === 0 && 'disabled' }
                                            onClick={selloutStore.Quantity >0 && this.makesellout}>制作报损
                                        </button>
                                        <button className={selloutStore.Quantity === 0 && 'disabled' }
                                            onClick={selloutStore.Quantity >0 && this.tosellout}>沽清
                                        </button> 
                                        <button 
                                            onClick={()=>{
                                                this.setState({
