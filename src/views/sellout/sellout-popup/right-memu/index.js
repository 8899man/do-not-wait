...
function Dishes({ selloutStore, dishes, handleClick }) { 
	return <li onClick={handleClick}>
            <div className="dishes-info" >
                 <div className={dishes.productName.length>5 ? 'title-small' : 'title-big'}>
                    {dishes.productName}
                </div>
                {dishes.productType === 1 && <div className="taocan">套餐</div>} </div>
...
class RightMemu extends React.Component{
    ...
    clickDishes(dishes) {   
        ...
        if(selloutStore.Takeeffect === true){  
            if(window.location.pathname === "/sellout"){
                ...
                //menuID：菜单ID(必填)
                 let _this=this;
                this.props.selloutStore.getselloutMessage(dishes.menuID,function(messageobj){
    ...
    reachValue=(e)=>{
            ...
            this.setState({recordValue:value});
             if(selloutStore.Takeeffect === true){ 
                    browserHistory.push('/sellout/after'); //生效后   
                    selloutStore.getDishesList({categoryID:'0',searchCode: e.target.value});
                }else if(selloutStore.Takeeffect === false){ 
                    selloutStore.getDishesList({categoryID:'0',searchCode: e.target.value});
                } 
    }
        ...
        return (
            <div id="right-memu">
                <div className="dishes-category">
                    <Tabs   activeKey={selloutStore.firstCategoryID} 
                            ...
                            {selloutStore.firstCategoryList.map((category) => {
                                return <TabPane tab={category.categoryName} key={category.categoryID} ></TabPane>
                            })}
                    </Tabs>
                </div>
                ...
                <div className="dishes-nav">
                    <div className='nav-tabs'>
                        <Tabs   activeKey={selloutStore.secondCategoryID} 
                                onTabClick={(categoryID) => { 
                                    this.setState({recordValue:''}) 
                                    selloutStore.recordValue="";
                                    if(selloutStore.Takeeffect === true){ 
                    ...
                    <div className='nav-search'>
                        <input  type="text" placeholder="搜索菜品"  onChange={this.reachValue}
               ...
               <ul className="dishes-list"> 
                    <MyScroll>
                        {selloutStore.dishesList.map((dishes,index) => {    
                           return <Dishes key={index} selloutStore={selloutStore} dishes={dishes} handleClick={this.clickDishes.bind(this, dishes)}  />
                        })} 
                        {selloutStore.dishesList.length === 0 && <div className="empty-holder">暂无菜品</div> }
