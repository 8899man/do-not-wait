 ...
 handleOk=()=>{      
    ...
    // }else{
        messagelist.map(function(obj){   
            if(obj.changevalue === true){//取出被改动过的数据  
                 curbSaleList.push({"optionID":obj.optionID,"quantity":obj.initFloatQuantity*1+""});  
            } 
            return obj;
        }); 
        let menuID=selloutStore.savemenuID;
        let  isAccOptionCureSale;//是否按规格沽清（必填，如果没有规格就填false）
        if(Number(this.state.type) === 1 || Number(this.state.type) === 2 ){//1：有规格按规格 2：有规格不按规格 
            isAccOptionCureSale=this.state.isCureSale;
        }else{//0：无规格 
            isAccOptionCureSale=false;
        }   
        this.props.selloutStore.addmessagevalue(curbSaleList,menuID,isAccOptionCureSale,()=>{
  ...
  render() {  
    let sumcount=0;
    let account = sessionStorage.getItem('account') && JSON.parse(sessionStorage.getItem('account'));   
    let  selloutStore=this.props.selloutStore;
    let  messagelist=selloutStore.selloutmessageList; 
    let  messageobj=selloutStore.messageobj; 
    let standard;
    ...
    if(messagelist.length) {  
        messagelist.forEach((mnews,index) => {    
            if(mnews.exchangeRate === 1){  
                ...
                reservenum = messageobj.standardOptionBookingSum === null ? 0: (messageobj.standardOptionBookingSum*1).toFixed(2)*1;//不按规格的预订数量
    ...
    return (
       <div>
          <Modal title={selloutStore.menuname}
              visible={true} 
              closable={false}
              maskClosable={false} 
              onCancel={this.handleCancel}
              onOk={this.handleOk}  
              width={840} wrapClassName="update-estimate-popup-modal"  
            >
          <div className="estimate-main">
            <Scrollbars>
              <div className="update-estimate-left">
                {(Number(this.state.type) === 1 && messageobj.accOptionCureSale === true)&& //1：有规格按规格
                <div className="update-left-content-yesan"> 
                    <div className="update-left-how">
                        <p className="update-how-span">共 {messagelist.length} 个规格 </p> 
                        {(() =>{ 
                            if(messagelist.length) {
                                let button;
                                messagelist.forEach((mnews,index) => {    
                                    if(mnews.exchangeRate === 1){  
                                            ...
                                            <p className="update-how-botton">
                                                <i className="iconfont icon-icon_checkbox_sel-" onClick={()=>{ 
                                                    this.setState({
                                                        type:'2',//2：有规格不按规格
                                                        isCureSale:"false" 
                                                    });   
                              ...
                              return messagelist.map((mnews,index) => { 
                                ...
                                return (
                                    <div key={index} className="update-left-each">
                                        <p className="update-left-type">{mnews.optionName}</p>
                                        <div className="update-left-number">
                                        {isDinner && <span>预订数量：<em>{mnews.bookingSum}</em></span>}
                                        <span>
                                            沽清数量 
                                            <input 
                                                type="text"  
                                                ...
                                                value={quantity || ''}
                                                onKeyDown={(e)=>{ 
                                                        if( messagelist[index].changevalue === false){
                                                            e.target.value="";
                                                        } 
                                                }}
                                                onChange={(e)=>{    
                                                    if(/^\d*$/.test(e.target.value)){ 
                                                        messagelist[index].initFloatQuantity=e.target.value; 
                                                        messagelist[index].changevalue=true;
                                                    }  
                                                    }}
                                                className={selloutStore.inputIndex === (sumcount-1) && 'changebg'} 
                                                onFocus={(e)=>{
                                                    selloutStore.inputIndex = e.target.id -1 ; 
                  ...
                  {(Number(this.state.type) === 2  && messageobj.accOptionCureSale === false) && //2：有规格不按规格
                  <div className="update-left-content-noan">
                      <div className="update-left-how">
                        <p className="update-how-span">标准规格：{selloutStore.standard} </p>
                        <p className="update-how-botton">
                          <i className="buan"  
                            onClick={()=>{
                                this.setState({
                      ...
                      <div className="update-left-each">  
                        ...
                        <input type="text"  
                            ...
                            onKeyDown={(e)=>{ 
                                if(messagelist[selloutStore.subscript].changevalue === false){
                                    e.target.value="";
                                } 
                            }} 
                            onChange={(e)=>{   
                                if(/^\d*$/.test(e.target.value)){ 
                                    messagelist[selloutStore.subscript].initFloatQuantity=e.target.value;
                                    selloutStore.estimatenum=e.target.value;
                                    messagelist[selloutStore.subscript].changevalue=true; 
                                }
                            }} 
                        />
                    </div>
              ...
              <div className="update-estimate-right">
                <div className="update-right-keyboard">
                  <CommonKeyboardNum  Whetherpoint={false} getvalue={(value)=>{ 
                      
                      let current = this.state.current; //1：有规格按规格 的时候文本框的下标 
                      let initFloatQuantity = messagelist[current].initFloatQuantity || ''; // 1：有规格按规格 沽清数量 称重 =true 的取值

    
                      let estimatenum = selloutStore.estimatenum || '';//2：有规格不按规格 沽清数量 也要判断称重【true/false】的取值
    
                      let initFloatQuantity1 =  messagelist[current].initFloatQuantity || ''; //0：没有规格 沽清数量 称重 =true 的取值
                        
                      if(Number(this.state.type) === 1 ){ //1：有规格按规格   
                        if(value === "click"){ //数字键盘点击删除    
                            messagelist[current].initFloatQuantity = (initFloatQuantity+'').substring(0,(initFloatQuantity+'').length-1); 
                            ...
                            if( messagelist[current].emptyvalue === false){
                              ...
                              if (messagelist[current].initFloatQuantity <= 100000000) { 
                                messagelist[current].initFloatQuantity = initFloatQuantity+value; 
                              } else {
                                message.destroy();
                                message.info('已达到最大值');
                      ...
                      }else if(Number(this.state.type) === 2){ //2：有规格不按规格  
                          if(value === "click"){  //数字键盘点击删除        
                                messagelist[selloutStore.subscript].initFloatQuantity =(estimatenum+'').substring(0,(estimatenum+'').length-1); 
                                 
                                selloutStore.estimatenum  = (estimatenum+'').substring(0,(estimatenum+'').length-1); 
                                messagelist[selloutStore.subscript].changevalue = true;  //说明数据有改动过 
                            }else{ //追加数据 
                                // if(value === "."){
                                ...
                                //     }else{
                                        // messagelist[selloutStore.subscript].initFloatQuantity = estimatenum+value; 
                                    // }  
                                if( messagelist[selloutStore.subscript].emptyvalue === false){
                                    messagelist[selloutStore.subscript].initFloatQuantity = ""; 
                                    messagelist[selloutStore.subscript].initFloatQuantity =value;
                                    messagelist[selloutStore.subscript].emptyvalue= true; 
                                    selloutStore.estimatenum=value;
                                }else{

                                  if ( messagelist[selloutStore.subscript].initFloatQuantity <= 100000000) { 
                                    messagelist[selloutStore.subscript].initFloatQuantity = estimatenum+value; 
                      ...
                      }else if(Number(this.state.type) === 0){//0：没有规格 
                            if(value === "click"){  //数字键盘点击删除 
                                messagelist[current].initFloatQuantity = (initFloatQuantity1+'').substring(0,(initFloatQuantity1+'').length-1);  
