...
class CalendarChoose extends React.Component{
    ...
    constructor(props, context) {
        ...
        this.state = {
            Calendar:'',//日历控件
            formattime: this.props.selloutStore.fillindate === "" ? formatTime() : this.props.selloutStore.fillindate,//获取当前时间
            foo:this.props.selloutStore.showright === true ? 'icon-right active-i' :  'dis-none'//控制是否可以往右边点
        }  
      ...
      return (
        <div className="estimate-left-calendar">
              <div className="icon-left active-i" onClick={this.nextdate}>
                <i className="iconfont icon-home_title_arrow_left"></i>
                <span className="previous-day" >前一日</span>
              </div>
              <input type="text" className="calendar-input" value={this.state.formattime}   readOnly  
                     onClick={()=>{
                        this.setState({Calendar:<CalendarPopup  maxTime={new Date(formatTime())}   changetime={new Date(this.state.formattime)} 
                                                    calendarModalCancel={()=>{
              ...
              <div  className={this.state.foo} onClick={this.lastdate}>
                ...
                <i className="iconfont icon-home_title_arrow_right"></i>
              </div> 
              {this.state.Calendar}
          </div>
