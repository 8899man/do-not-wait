...
class OperationTypePopup extends React.Component {
 ...
 //点击确定按钮
 handleOk=()=>{
    this.props.selloutStore.changeOperateType(this.state.TypeID, this.state.typeName);
       ...
       <div>
          <Modal title="选择操作类型" 
             ...
             closable={false}
              ...
              onOk={this.handleOk}
              width={840} wrapClassName="operation-type-popup"
            >
                ...
                <MyScroll > 
                    {(() =>{
                        if(operatetype.length) {
                            ...
                            return <div key={operate.OperationType}  className={classnames({
                                'each-type-data': true, 
                                'type-checked':selloutStore.savetypeid === operate.OperationType ? true : false
                            })}  
                             onClick={()=>{   
