...
class GqEstimatePopup extends React.Component {
 ...
 handleOk=()=>{
  ...
  if(this.props.selloutStore.reasontext.reasonName === "" && this.state.recordValue.trim() === ""){
       message.warn("请选择原因"); 
       return; 
      ...
      message.warn("输入的原因不能超过100个字"); 
       return; 
        ...
        let changeReason = this.props.selloutStore.reasontext.reasonName === "" ? textareaReason : this.props.selloutStore.reasontext.reasonName; 
 ...
 reachValue=(e)=>{ 
  ...
  render() {
    ...
    return (
       <div>
          <Modal title={selloutStore.addorreducename}
              visible={true}
              closable={false}
              maskClosable={false}
              onCancel={this.handleCancel}
              onOk={this.handleOk}
              width={840} wrapClassName="estimate-gq-popup"
            >
           <div className="estimate-gq-title">{selloutStore.afterobj.productName}</div>
                    ...
                    {(() =>{
                      if(reasonlist.length) {
                          ...
                          return <div 
                                    ...
                                    className={classnames({'each-reason': true, 'each-checked': reason.selected})}  
                                    onClick={()=>{ 
                                      selloutStore.selectreason(reason.DictionaryID);  //选中
                                      selloutStore.getreasontext(reason.DictionaryID,reason.DictionaryName);  //获取文本 
