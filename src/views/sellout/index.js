...
function SelloutLb({ selloutStore, sellout,handleClick}) {
    ...
    return (
        <li 
            onClick={handleClick} 
            className={sellout.selected === true ? 'each-data click-data' : 'each-data'}
        > 
                    ...
                    <TabPane tab="按日沽清" key="0" className="estimate-clear-tab">
                        <div className="estimate-left">
                            <div className="estimate-left-search">
                                ...
                                <input type="text" placeholder="请输入商品编码/名称/拼音码查询"  className="estimate-left-input" value={this.state.recordValue} onChange={this.reachValue}></input>
                                <button  className="search-box" 
                                    onClick={()=>{  
                                        if(this.selloutStore.Takeeffect === true){//判断是否选中有数据
                                            browserHistory.push('/sellout/after'); //生效后去到生效后的页面
                                ...
                                <div className="estimate-list-button">
                                    ...
                                    <div className="estimate-list"> 
                                            <div className="estimate-scroll">
                                                <MyScroll > 
                                                     {this.selloutStore.selloutList.length !== 0 ? this.selloutStore.selloutList.map((sellout,index) => { 
                                        ...
                                        <button className={classnames({
                                            ...
                                            onClick={()=>{  
                                                if(this.selloutStore.selloutList.length !== 0){
                                                   let flagt;
                                                    this.selloutStore.selloutList.map((sellout,index)=>{//判断是否选中有数据
                                                    ...
                                                    if(this.selloutStore.Takeeffect === true){//判断是否生效
                                                        browserHistory.push('/sellout/after'); //生效后去到生效后的页面
                                                    }else if(this.selloutStore.Takeeffect === false){
                                                        if(flagt !== true){
                                                            message.destroy();
                                                            ...
                                                            let _this=this;
                                                            this.props.selloutStore.getselloutMessage(this.state.menuID,function(messageobj){
                                                                if(messageobj.accOptionCureSale === true){
                                                                        _this.props.selloutStore.optionIDSum= messageobj.optionIDSum > 0 ? 1: 0; 
                                                    ...
                                                    if(this.selloutStore.selloutList.length !== 0){
                                                        ...
                                                        if(this.selloutStore.Takeeffect === true){//判断是否生效
                                                            ...
                                                            if(flagt !== true){
                                                                ...
                                                                this.props.selloutStore.delselloutlist(this.state.mappingID);//执行删除的action 12代表是删除  
                                                            }  
                                                        }   
                                                    }  
                                            }}>删除
