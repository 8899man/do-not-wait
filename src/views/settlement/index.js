...
class Settlement extends Component {
                ...
                promptpopup: (
                  <PromptPopup
                    ...
                    onOk={() => {
                      ...
                      settlementStore.amountReceived = parseFloat(settlementStore.amountReceived) - parseFloat(recordAmount); //重新计算已收金额
                      settlementStore.waitCollectAmount = parseFloat(settlementStore.waitCollectAmount) + parseFloat(recordAmount); //重新计算待收金额
  ...
  render() {
    let settlementStore = this.props.settlementStore;
    let feedback = settlementStore.feedback;
    let orderInfo = settlementStore.orderInfo;
    let morePopup;
    let adjustProduct;
    let discountPopup;
                        ...
                        () => {
                          _this.setState({
                            discountPricePopup: (
                              <DiscountPrice
                                alert={
                                  '你本次可减免金额最多为' + settlementStore.bigDecimal
                                }
                                title={ele.text}
                                onOk={value => {
