...
class TuikuanPopup extends React.Component {
  render() {
    ...
    return <Modal title={this.props.title+"退款"} 
        visible={true} 
        maskClosable={false}
        onOk={()=>{
            if(this.props.onOk){
        ...
        width={840} wrapClassName="card-tuikuan-popup-modal"
        onCancel={()=>{
        ...
        <div className="tuikuan-block">
            <div className="tuikaun-money">
                <p>本次退款</p>
                <div className="money-content">{this.props.title}<i>{this.props.textValue}</i>元</div>
            </div>
            ...
            <textarea type="text" placeholder="可输入自定义赠菜原因" className="tuikaun-remarks">
            </textarea>
        </div>
    </Modal>
  }
