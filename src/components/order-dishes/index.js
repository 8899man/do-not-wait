  ...
  return (
    ...
    >
      <div className="row-one">
        {dishesStore.dishesStage === 'spell' && isSpellChild ? (
          <span
            className="yichu"
            onClick={e => {
              e.stopPropagation();
        ...
        <span className="number">
          {dishesStore &&
            ...
            !(isSpellDishes && dishes.quantity === 1) &&
        ...
        {!dishes.isAddPrice &&
          dishes.price > -1 && (
            ...
            >
              ￥{Number((dishes.needWeigh ? dishes.expectedWeight : dishes.quantity) * dishes.price).toFixed(2)}
            </span>
          )}
        {dishes.isAddPrice &&
          dishes.price > 0 && (
...
class WillOrderList extends Component {
  ...
  //修改未下单菜品
  modifyWillOrder({ dishes, cartIndex, isSpellChild }) {
        ...
        <ul className="dishes-list">
          <Scrollbars>
            {dishesStore.dishesList.length ? (
              dishesStore.dishesList.map((dishes, index) => {
                return (
