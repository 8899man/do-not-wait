...
class DishesStore {
    ...
    getJSON({
      ...
      success: function(json) {
        if (json.code === 0 && json.data) {
          ...
          _this.orderInfo = [];
          _this.showFeedback({ status: 'warn', msg: `订单信息，${json.message}` });
        }
      }
    });
  ...
  //更新购物车菜品数量通知后端
  @action
  updateShoppingCart({ productID, variantID, optionID, changeQuantity, remaining }) {
    let _this = this;
  ...
  moveDishesFromSpellToCart = dishes => {
    ...
    this.shoppingCart.forEach(product => {
      if (product.assortedDishesList) {
        //当只有一个拼菜明细的时候取消拼菜模式
        if (product.assortedDishesList.length === 1) {
          this.cancelSpellStage();
          return;
        }
  ...
  get spellPrice() {
    let spellPrice = 0;
    this.shoppingCart.forEach(product => {
      if (product.assortedDishesList && product.assortedDishesList.length > 0) {
        product.assortedDishesList.forEach(dishes => {
          spellPrice += dishes.quantity * dishes.price;
        });
      }
    });
